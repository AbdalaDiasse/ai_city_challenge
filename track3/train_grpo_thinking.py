"""
Phase 3 (Thinking variant) — GRPO fine-tuning starting from the Thinking SFT checkpoint.

Key differences from train_grpo.py:
  - Base model: Qwen3-VL-8B-Thinking-unsloth-bnb-4bit (pre-trained CoT reasoning).
  - Starts from the Thinking SFT checkpoint (sft_thinking_v1/lora_adapter).
  - max_completion_len=1024 — Thinking model needs more tokens for <think> chains.
  - BCQ/MCQ reward functions still extract Yes/No and A-D from the text AFTER the
    think block, consistent with how inference.py's strip_think() works.

Reward functions (same structure as train_grpo.py):
  - bcq / mcq:              exact-match (0 or 1)
  - bcq_openended:          yes/no prefix match (0 or 1)
  - mcq_openended:          letter prefix match (0 or 1)
  - temporal_localization:  temporal IoU (0–1)
  - open-ended tasks:       ROUGE-L (0–1)

Usage (multi-GPU via torchrun):
    torchrun --nproc_per_node 4 track3/train_grpo_thinking.py

Key env vars:
    WORK   — storage root (default: /leonardo_work/AIH4A_syrate)
    RUN    — experiment name (default: grpo_thinking_v1)
"""
...
# Default SFT checkpoint from Thinking SFT run
DEFAULT_SFT_CKPT = f"{WORK}/checkpoints/track3_sft/sft_thinking_v1/lora_adapter"
...
def parse_args():
    ...
    p.add_argument("--sft_ckpt",          default=DEFAULT_SFT_CKPT,
                   help="Thinking SFT LoRA adapter to start GRPO from")
    p.add_argument("--run_name",          default=os.environ.get("RUN", "grpo_thinking_v1"))
    p.add_argument("--epochs",            type=int,   default=1)
    p.add_argument("--lr",                type=float, default=5e-6)
    p.add_argument("--batch_size",        type=int,   default=1)
    p.add_argument("--num_generations",   type=int,   default=4,
                   help="Completions per prompt for GRPO gradient estimate")
    p.add_argument("--max_prompt_len",    type=int,   default=2048)
    p.add_argument("--max_completion_len", type=int,  default=1024,
                   help="Thinking model needs longer budget for <think> chains")
...
class GRPODataset(TorchDataset):
    ...
    Exposes per-record:
      ...
      - "answers":    clean ground-truth (think blocks stripped)
      - "task_types": task name string for reward routing
    """
    def __init__(self, records: list[dict]):
        ...
        for rec in records:
            ...
            # Replace system prompt with Thinking-model version
            prompt_msgs = [
...
def _strip_think(text) -> str:
    ...
    text = str(text)
    # Two-pass: remove complete blocks, then remove unclosed block to end
...
def _rouge_l(pred: str, ref: str) -> float:
    if not pred.strip() or not ref.strip():
        return 0.0
    pred_tokens = pred.lower().split()[:200]
    ref_tokens  = ref.lower().split()[:200]
...
def load_model_from_sft(sft_ckpt: str):
    print(f"Loading base Thinking model: {MODEL_NAME}")
    model, tokenizer = FastVisionModel.from_pretrained(
        model_name=MODEL_NAME,
        load_in_4bit=True,
        use_gradient_checkpointing="unsloth",
    )
    print(f"Applying Thinking SFT adapter: {sft_ckpt}")
    model = PeftModel.from_pretrained(model, sft_ckpt, is_trainable=True)
    print("Thinking SFT adapter loaded and set trainable for GRPO.")
...
def main():
    ...
    print("Starting GRPO training (Thinking model)...")
    trainer.train(resume_from_checkpoint=args.resume or None)

    adapter_dir = output_dir / "lora_adapter"
    model.save_pretrained(str(adapter_dir))
    tokenizer.save_pretrained(str(adapter_dir))
    print(f"GRPO Thinking adapter saved → {adapter_dir}")
